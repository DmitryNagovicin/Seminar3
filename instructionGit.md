...
## Удаление ветки
   ...
   - # Конфликт слияния веток.
    ...
    Если одна и таже строка в разных версиях записана по разному, то возникает конфликт.
    Чистый гит автоматически сохраняет оба изменения. Далее требуется ввести вручную нужные правки и сделать коммит.

    VSCode дает возможность выбрать какое изменение следует сохранить (входящее, существующее или оба).
